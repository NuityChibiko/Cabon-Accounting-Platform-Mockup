...
import {
  scope3DetailsData,
  scope3MonthlyDetailsData,
  COLORS,
} from "../data/mockData";

interface Scope3DetailsPageProps {
  onBack: () => void;
  year: string;
}

const Scope3DetailsPage: React.FC<Scope3DetailsPageProps> = ({
  onBack,
  year,
}) => {
  ...
  const monthlyData =
    scope3MonthlyDetailsData[
      selectedYear as keyof typeof scope3MonthlyDetailsData
    ];
      ...
      <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-md">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            ...
            >
              {yearData.categories.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
      ...
      <ResponsiveContainer width="100%" height={350}>
        ...
        >
          ...
          <Legend
            ...
            align="right"
          />
          <Bar dataKey="Waste Gen" stackId="a" fill={COLORS[0]} />
          <Bar dataKey="Use of Sold Product" stackId="a" fill={COLORS[1]} />
          <Bar dataKey="Fuel Related" stackId="a" fill={COLORS[2]} />
          <Bar dataKey="Employee Commuting" stackId="a" fill={COLORS[3]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
  ...
  return (
    <>
      <DashboardHeader
        title="Scope 3 Emissions Details"
        subtitle={`รายละเอียดการปล่อยก๊าซเรือนกระจกขอบเขตที่ 3`}
        timeRange={timeRange}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-md flex flex-col justify-center items-center">
          <h3 className="text-lg font-medium text-gray-500">
            Total Scope 3 Emissions ({parseInt(selectedYear) + 543})
          </h3>
...
export default Scope3DetailsPage;
